...
pub(super) fn plugin(app: &mut App) {
    app.add_plugins((
        ...
        .add_systems(Update, (Player::rotate_model, Player::animate));
}
...
impl Player {
    ...
    fn on_scene_ready(
        ...
        for child in children.iter_descendants(event.entity) {
            let Ok(mut aplayer) = aplayer.get_mut(child)
            else { continue };
            ...
            cmd.entity(child).insert(AnimationGraphHandle(model.graph.clone()));
        }
    ...
    fn animate(
        ...
        let Some(aplayer_entity) = model.aplayer
        else { return Ok(()) };
        ...
        if player.2.xz().length_squared() == 0.0 {
            ...
            if !aplayer.is_playing_animation(model.slide_start) {
                ...
                } else if let Some(anim) = aplayer.animation(model.slide_start) && anim.is_finished() {
